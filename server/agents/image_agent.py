...
class ImageGenerationAgent:
    ...
    def __createAgent(self) -> Agent:
        ...
        return Agent(
            ...
            goal="Generate colorful marketing ready images for products that can be shared on social medial",
            backstory="""You are a talented marketing specialist with expertise in curating 
            colorful,images that appeal consumers. You excel at transforming story 
    ...
    async def _generate_image(self, prompt: str, output_path: str) -> str:
        ...
        try:
            ...
            task = Task(
                description=f"""Generate a colorRealistic, Advertisement like Image story page.
                
                Scene description: {prompt}
                
                Requirements:
                1. Style: Colorful and ad like
    ...
    async def generate_images(self, story: StoryOutput, output_dir: str) -> List[str]:
        ...
        for page in story.pages:
            ...
            filename = f"page_{page.page_number}_{story.title.lower().replace(' ', '_').replace(':','_')}.png"
